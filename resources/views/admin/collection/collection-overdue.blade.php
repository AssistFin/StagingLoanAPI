...
<script>
    function fetchCODs(page = 1) {
        const dateRange = $('#date_range').val();
        const fromDate = $('#from_date').val();
        const toDate = $('#to_date').val();
        const searchTerm = $('#c-od-search-input').val();
        const dpdFrom = $('#dpd_from').val();
        const dpdTo = $('#dpd_to').val();

        $.ajax({
            url: "{{ route('admin.collection.overdue') }}",
            type: "GET",
            data: {
                page: page,
    ...
    $(document).ready(function () {
        $('#date_range').on('change', () => fetchCODs());
        $('#c-od-search-input').on('keyup', () => fetchCODs());
        ...
        $('#cOverduePaginationLinks').on('click', '.pagination .page-item .page-link', function (e) {
            e.preventDefault();
        ...
        $('#cod_export').on('click', function () {
            const params = {
                date_range: $('#date_range').val(),
                from_date: $('#from_date').val(),
                to_date: $('#to_date').val(),
                search: $('#c-od-search-input').val(),
                dpd_from: $('#dpd_from').val(),
                dpd_to: $('#dpd_to').val(),
                export: 'csv'
            };
            const query = $.param(params);
            window.location.href = "{{ route('admin.collection.overdue') }}?" + query;
        });
        ...
        $('#filterByDPD').on('click', function() {
            ...
            fetchCODs(); // this will include the DPD params automatically
        });
    });
