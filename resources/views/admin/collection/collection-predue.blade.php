...
@push('script')
<script>
        const searchInput = document.getElementById('c-pd-search-input');
        const cPredueTable = document.getElementById('cPredueTable');
        const cPreduePaginationLinks = document.getElementById('cPreduePaginationLinks');
        let searchTimeout;
        ...
        function fetchLeads(searchTerm) {
            const url = `/admin/collection/collection-predue?search=${searchTerm}`;
            ...
            .then(html => {
                ...
                const newTbody = tempElement.querySelector('#cPredueTable');

                if (newTbody) {
                    cPredueTable.innerHTML = newTbody.innerHTML;
                } else {
                    cPredueTable.innerHTML = '<tr><td colspan="11">No data found.</td></tr>';
                }
                
                // Update pagination links
                if (searchTerm) {
                    cPreduePaginationLinks.innerHTML = newPagination ? newPagination.innerHTML : '';
                } else {
                    cPreduePaginationLinks.innerHTML = initialPaginationHTML;
                }
            })
            .catch(error => {
