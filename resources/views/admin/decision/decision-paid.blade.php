                ...
                <div class="card-body">
                    <div class="flex flex-wrap items-center gap-3 py-4">
                        <div class="filter-row">
                            ...
                            <input type="text" id="d-paid-search-input" class="form-control" placeholder="Search by Name / Mobile No / Loan App No / Email..." style="max-width: 400px;">

                            <button type="button" id="all_paid_export" class="btn btn-primary form-control">Export CSV</button>
                            ...
                            @forelse($leads as $lead)
                                <tr>
                                    <td style="cursor: pointer">
                                        <a href="{{route('admin.leads.verify', base64_encode($lead->loanApplication->id))}}"><i class="fas fa-eye"></i></a>
                                    </td>
                                    <td>{{ $lead->loanApplication->loan_no }}</td>
                                    <td>{{ $lead->user ? $lead->user->firstname . " " . $lead->user->lastname : '' }}</td>
                                    <td>{{ $lead->user ? $lead->user->mobile : '' }}</td>
                                    <td>{{ number_format(optional($lead->loanApplication->loanApproval)->approval_amount ?? 0, 0) }}</td>
                                    <td>{{ $lead->collection_amt }}</td>
                                    <td>{{ $lead->collection_date }}</td>
                                    <td>{{ $lead->status }}</td>
                                </tr>
                            @empty
                                <tr>
...
<script>
        const searchInput = document.getElementById('d-paid-search-input');
        const dateRangeSelect = document.getElementById('date_range');
        const fromDateInput = document.getElementById('from_date');
        const toDateInput = document.getElementById('to_date');
        const customDateSection = document.getElementById('customDateSection');
        const dpaidTable = document.getElementById('dpaidTable');
        ...
        function fetchLeads(searchTerm = '') {
            ...
            const url = `/admin/decision/decision-paid?${params.toString()}`;
            ...
            .then(html => {
                ...
                const newTbody = tempElement.querySelector('#dpaidTable');
                const newPagination = tempElement.querySelector('#dClosePaginationLinks');

                dpaidTable.innerHTML = newTbody ? newTbody.innerHTML : '<tr><td colspan="11">No data found.</td></tr>';
        ...
        $(document).ready(function () {
            ...
            $('#from_date, #to_date').datepicker({
                ...
                if (fromDate && toDate) {
                    fetchLeads($('#d-paid-search-input').val().trim());
                }
            });

            $('#all_paid_export').on('click', function () {
                const params = {
                    search: $('#d-paid-search-input').val(),
                ...
                window.location.href = "{{ route('admin.decision.paid') }}?" + query;
            });
