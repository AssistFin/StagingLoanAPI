                                ...
                                @forelse($groupedLeads as $userId => $userLoans)
                                    @foreach($userLoans as $index => $lead)
                                        <tr>
                                            ...
                                            <td>{{ $lead->collections->first()->total_paid ?? 0 }}</td>
                                            <td>{{ $lead->collections->first()->last_payment_date ?? null }}</td>
                                        </tr>
                                    @endforeach
                                @empty
                                    <tr>
...
<script>
    function fetchReports(page = 1) {
        ...
        $.ajax({
            url: "{{ route('admin.osreport.cindex') }}",
            type: "GET",
            data: {
                page: page,
    ...
    $(document).ready(function () {
        ...
        $('#report_export').on('click', function () {
            ...
            window.location.href = "{{ route('admin.osreport.cindex') }}?" + query;
        });
    });
    </script>
@endpush