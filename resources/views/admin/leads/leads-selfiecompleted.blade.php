                ...
                <div class="card-body">
                    <div class="flex flex-wrap items-center gap-3 py-4">
                        <div class="filter-row">
                            ...
                            <input type="text" id="selfiecompleted-lead-search-input" class="form-control" 
                            ...
                            <button type="button" id="selfiecompleted_lead_export" class="btn btn-primary form-control">Export CSV</button>
                        </div>
                    ...
                    <div class="table-responsive--md  table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody id="selfiecompletedLeadsTable">
                        ...
                        <br>
                        <br>  
                        <div class="sticky-pagination" id="selfiecompletedpaginationLinks">
                            {{ $leads->links() }}
                        </div>
                    </div>
                </div>
...
<script>
    function fetchselfiecompletedLeads(page = 1) {
        const dateRange = $('#date_range').val();
        const fromDate = $('#from_date').val();
        const toDate = $('#to_date').val();
        const missingInfo = $('#missing_info').val();
        const searchTerm = $('#selfiecompleted-lead-search-input').val();

        $.ajax({
            url: "{{ route('admin.leads.selfiecompleted') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                $('#selfiecompletedLeadsTable').html($(response).find('#selfiecompletedLeadsTable').html());
                $('#selfiecompletedpaginationLinks').html($(response).find('#selfiecompletedpaginationLinks').html());
    ...
    $(document).ready(function () {
        $('#date_range, #missing_info').on('change', () => fetchselfiecompletedLeads());
        $('#selfiecompleted-lead-search-input').on('keyup', () => fetchselfiecompletedLeads());
        ...
        $('#from_date, #to_date').on('change', function () {
            ...
            if (fromDate && toDate && new Date(toDate) >= new Date(fromDate)) {
                fetchselfiecompletedLeads();
            }
        });

        $('#selfiecompletedpaginationLinks').on('click', '.pagination .page-item .page-link', function (e) {
            e.preventDefault();
            ...
            try {
                ...
                fetchselfiecompletedLeads(page);
            } catch (error) {
                console.error("Invalid pagination URL", error);
            }
        });

        $('#selfiecompleted_lead_export').on('click', function () {
            const params = {
                date_range: $('#date_range').val(),
                from_date: $('#from_date').val(),
                to_date: $('#to_date').val(),
                missing_info: $('#missing_info').val(),
                search: $('#selfiecompleted-lead-search-input').val(),
                export: 'csv'
            };
            const query = $.param(params);
            window.location.href = "{{ route('admin.leads.selfiecompleted') }}?" + query;
        });
    });
</script>
@endpush