                ...
                <div class="card-body">
                    <div class="flex flex-wrap items-center gap-3 py-4">
                        <div class="filter-row">
                            ...
                            <select id="missing_info" name="missing_info" class="form-control">
                                <option value="">Select Missing Info</option>
                                <option value="kyc_details">KYC Details</option>
                                <option value="selfie_docs">Selfie Documents</option>
                                <option value="address_details">Address Details</option>
                                <option value="employment_details">Employement Details</option>
                                <option value="bank_details">Bank Details</option>
                            </select>

                            <input type="text" id="wbs-lead-search-input" class="form-control" 
                            ...
                            <button type="button" id="wbs_lead_export" class="btn btn-primary form-control">Export CSV</button>
                        </div>
                    ...
                    <div class="table-responsive--md  table-responsive">
                        ...
                        <br>
                        <br>  
                        <div class="sticky-pagination" id="wbspaginationLinks">
                            {{ $leads->links() }}
                        </div>
                    </div>
                </div>
...
<script>
    function fetchWbsLeads(page = 1) {
        const dateRange = $('#date_range').val();
        const fromDate = $('#from_date').val();
        const toDate = $('#to_date').val();
        const missingInfo = $('#missing_info').val();
        const searchTerm = $('#wbs-lead-search-input').val();

        $.ajax({
            url: "{{ route('admin.leads.wbs') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                $('#wbsLeadsTable').html($(response).find('#wbsLeadsTable').html());
                $('#wbspaginationLinks').html($(response).find('#wbspaginationLinks').html());
    ...
    $(document).ready(function () {
        $('#date_range, #missing_info').on('change', () => fetchWbsLeads());
        $('#wbs-lead-search-input').on('keyup', () => fetchWbsLeads());
        ...
        $('#wbspaginationLinks').on('click', '.pagination .page-item .page-link', function (e) {
            e.preventDefault();
        ...
        $('#wbs_lead_export').on('click', function () {
            const params = {
                date_range: $('#date_range').val(),
                from_date: $('#from_date').val(),
                to_date: $('#to_date').val(),
                missing_info: $('#missing_info').val(),
                search: $('#wbs-lead-search-input').val(),
                export: 'csv'
            };
            const query = $.param(params);
            window.location.href = "{{ route('admin.leads.wbs') }}?" + query;
        });
    });
</script>
@endpush