                ...
                <div class="card-body">
                    <div class="flex flex-wrap items-center gap-3 py-4">
                        <div class="filter-row">
                            ...
                            <input type="text" id="pendingselfie-lead-search-input" class="form-control" 
                            ...
                            <button type="button" id="pendingselfie_lead_export" class="btn btn-primary form-control">Export CSV</button>
                        </div>
                    ...
                    <div class="table-responsive--md  table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody id="pendingselfieLeadsTable">
                        ...
                        <br>
                        <br>  
                        <div class="sticky-pagination" id="pendingselfiepaginationLinks">
                            {{ $leads->links() }}
                        </div>
                    </div>
                </div>
...
<script>
    function fetchpendingselfieLeads(page = 1) {
        const dateRange = $('#date_range').val();
        const fromDate = $('#from_date').val();
        const toDate = $('#to_date').val();
        const missingInfo = $('#missing_info').val();
        const searchTerm = $('#pendingselfie-lead-search-input').val();

        $.ajax({
            url: "{{ route('admin.leads.pendingselfie') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                $('#pendingselfieLeadsTable').html($(response).find('#pendingselfieLeadsTable').html());
                $('#pendingselfiepaginationLinks').html($(response).find('#pendingselfiepaginationLinks').html());
    ...
    $(document).ready(function () {
        $('#date_range').on('change', () => fetchpendingselfieLeads());
        $('#pendingselfie-lead-search-input').on('keyup', () => fetchpendingselfieLeads());
        ...
        $('#from_date, #to_date').on('change', function () {
            ...
            if (fromDate && toDate && new Date(toDate) >= new Date(fromDate)) {
                fetchpendingselfieLeads();
            }
        });

        $('#pendingselfiepaginationLinks').on('click', '.pagination .page-item .page-link', function (e) {
            e.preventDefault();
            ...
            try {
                ...
                fetchpendingselfieLeads(page);
            } catch (error) {
                console.error("Invalid pagination URL", error);
            }
        });

        $('#pendingselfie_lead_export').on('click', function () {
            const params = {
                date_range: $('#date_range').val(),
                from_date: $('#from_date').val(),
                to_date: $('#to_date').val(),
                search: $('#pendingselfie-lead-search-input').val(),
                export: 'csv'
            };
            const query = $.param(params);
            window.location.href = "{{ route('admin.leads.pendingselfie') }}?" + query;
        });
    });
</script>
@endpush