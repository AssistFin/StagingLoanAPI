                ...
                <div class="card-body">
                    <div class="flex flex-wrap items-center gap-3 py-4">
                        <div class="filter-row">
                            ...
                            <input type="text" id="pendingkyc-lead-search-input" class="form-control" 
                            ...
                            <button type="button" id="pendingkyc_lead_export" class="btn btn-primary form-control">Export CSV</button>
                        </div>
                    ...
                    <div class="table-responsive--md  table-responsive">
                        ...
                        <br>
                        <br>  
                        <div class="sticky-pagination" id="pendingkycpaginationLinks">
                            {{ $leads->links() }}
                        </div>
                    </div>
                </div>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('admin.leads.pendingkyc'))
...
<script>
    function fetchpendingkycLeads(page = 1) {
        const dateRange = $('#date_range').val();
        const fromDate = $('#from_date').val();
        const toDate = $('#to_date').val();
        const missingInfo = $('#missing_info').val();
        const searchTerm = $('#pendingkyc-lead-search-input').val();

        $.ajax({
            url: "{{ route('admin.leads.pendingkyc') }}",
            type: "GET",
            data: {
                page: page,
            ...
            success: function(response) {
                $('#pendingkycLeadsTable').html($(response).find('#pendingkycLeadsTable').html());
                $('#pendingkycpaginationLinks').html($(response).find('#pendingkycpaginationLinks').html());
    ...
    $(document).ready(function () {
        $('#date_range').on('change', () => fetchpendingkycLeads());
        $('#pendingkyc-lead-search-input').on('keyup', () => fetchpendingkycLeads());
        ...
        $('#from_date, #to_date').on('change', function () {
            ...
            if (fromDate && toDate && new Date(toDate) >= new Date(fromDate)) {
                fetchpendingkycLeads();
            }
        });

        $('#pendingkycpaginationLinks').on('click', '.pagination .page-item .page-link', function (e) {
            e.preventDefault();
            ...
            try {
                ...
                fetchpendingkycLeads(page);
            } catch (error) {
                console.error("Invalid pagination URL", error);
            }
        });

        $('#pendingkyc_lead_export').on('click', function () {
            const params = {
                date_range: $('#date_range').val(),
                from_date: $('#from_date').val(),
                to_date: $('#to_date').val(),
                search: $('#pendingkyc-lead-search-input').val(),
                export: 'csv'
            };
            const query = $.param(params);
            window.location.href = "{{ route('admin.leads.pendingkyc') }}?" + query;
        });
    });
</script>
@endpush