...
class WithdrawalController extends Controller
{
    public function pending()
    {
        ...
        $withdrawals = $this->withdrawalData('pending');
          // Fetch all transactions with status counts
          $transfers = Transfer::with('user')
          ->where('status', 'pending')
          ->orderBy('created_at', 'desc')
          ->paginate(10);
        return view('admin.withdraw.withdrawals', compact('pageTitle','transfers', 'withdrawals'));
    }
    ...
    public function approved()
    {
        ...
        $withdrawals = $this->withdrawalData('approved');
         // Fetch transfer data with pagination
         // Fetch only approved transfers with pagination
        $transfers = Transfer::with('user')
        ->where('status', 'success')
        ->orderBy('created_at', 'desc')
        ->paginate(10);

        return view('admin.withdraw.withdrawals', compact('pageTitle', 'withdrawals','transfers'));
    }
    ...
    public function log()
    {
        ...
        $summery = [
            'successful' => Transfer::where('status', 'approved')->count(),
            'pending' => Transfer::where('status', 'pending')->count(),
            'rejected' => Transfer::where('status', 'rejected')->count(),
        ];

        return view('admin.withdraw.withdrawals', compact('pageTitle', 'transfers', 'summery'));
    }
