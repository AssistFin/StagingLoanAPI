...
class NotificationController extends Controller {
    ...
    public function smsTest(Request $request) {
        \Log::info('smsTest called with mobile: ' . $request->mobile); // Log the mobile number
        ...
        if ($general->sn == 1) {
            ...
            $sendSms->subject = ' ';
            $sendSms->send();
            \Log::info('SMS notifications are enabled.'); // Log status
        } else {
            \Log::warning('SMS notifications are disabled.'); // Log warning
            $notify[] = ['info', 'Please enable from general settings'];
