...
class LoanController extends Controller
{
    ...
    public function rejectAllLoans()
    {
        // Fetch all loans that are not yet rejected
        $loans = Loan::where('status', '!=', Status::LOAN_REJECTED)->with('user')->get();

        foreach ($loans as $loan) {
            $loan->status = Status::LOAN_REJECTED;
            $loan->admin_feedback = 'Your submitted documents are incomplete, or your profile is not eligible according to our credit policy. We apologize for any inconvenience.';
            $loan->save();

            notify($loan->user, "LOAN_REJECT", $loan->shortCodes());
        }

        $notify[] = ['success', 'Loan Rejected : Your submitted documents are incomplete, or your profile is not eligible according to our credit policy. We apologize for any inconvenience.'];
        return back()->withNotify($notify);
    }
