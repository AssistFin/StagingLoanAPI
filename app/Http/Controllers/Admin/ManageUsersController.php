...
class ManageUsersController extends Controller {
    ...
    public function bankDetails($id) {
        ...
        $user = User::findOrFail($id);
        return view('admin.users.bank_detail', compact('pageTitle', 'user'));
    }

    public function deviceDetails($id) {
        $pageTitle = 'User Device Data';
        $user = User::findOrFail($id);
        return view('admin.users.device_data', compact('pageTitle', 'user'));
    }

    public function showContacts($id)
{
    $pageTitle = 'User Device Contacts Data';
    $user = User::findOrFail($id);
    $contacts = json_decode($user->contacts, true);

    return view('admin.users.contacts', compact('pageTitle','user', 'contacts'));
}
